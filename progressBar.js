...
function getProgressBar() {
    ...
    var filename = $("#filename").val().replace(/[ ]/g,"");//视频文件名
    var varietyid = $("#varietyid").val().replace(/[ ]/g,"");//视频系统分类
	var getuservideoid = $("#getuservideoid").val().replace(/[ ]/g,"");//视频用户分类
	var bl = true;
	if(filename.length == 0){   
	   alert("视频名称不能为空！");
	   bl = false;
	   window.location.href = "../usermanager/ftpvideoupload.jsp";
    }else if(varietyid.length == 0){
	   alert("系统分类不能为空！");
	   bl = false;
	   window.location.href = "../usermanager/ftpvideoupload.jsp";
    }else if(getuservideoid.length == 0){
	   alert("用户分类不能为空！");
	   bl = false;
	   window.location.href = "../usermanager/ftpvideoupload.jsp";
    }
	if(bl)
	{
			$.getJSON("http://222.74.218.3:8081/fileupload/getBar", {"t":timestamp}, function (json) {
			...
			if (bytesRead == contentLength && json.pContentLength!=0) {
				$("#close").show();
				...
				if (bytesReadGtMB == 0 && contentLengthGtMB == 0) {
					$("div#info").html("\u4e0a\u4f20\u901f\u5ea6:" + speed + "KB/秒,\u5df2\u7ecf\u8bfb\u53d6" + bytesReadToShow + "KB,\u603b\u5171\u5927\u5c0f" + contentLengthToShow + "KB.\u5b8c\u6210" + percent);
				} else {
					if (bytesReadGtMB == 0 && contentLengthGtMB == 1) {
						$("div#info").html("\u4e0a\u4f20\u901f\u5ea6:" + speed + "KB/秒,\u5df2\u7ecf\u8bfb\u53d6" + bytesReadToShow + "KB,\u603b\u5171\u5927\u5c0f" + contentLengthToShow + "MB.\u5b8c\u6210" + percent);
					} else {
						if (bytesReadGtMB == 1 && contentLengthGtMB == 1) {
							$("div#info").html("\u4e0a\u4f20\u901f\u5ea6:" + speed + "KB/秒,\u5df2\u7ecf\u8bfb\u53d6" + bytesReadToShow + "MB,\u603b\u5171\u5927\u5c0f" + contentLengthToShow + "MB.\u5b8c\u6210" + percent);
						}
					}
				}
			}
